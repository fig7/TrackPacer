...
fun timeToString(resources: Resources, timeInMS: Int64): String {
    ...
    return if(hrs > 0L) {
        val hrsStr  = String.format(Locale.ROOT, "%d", hrs)
        val minsStr = String.format(Locale.ROOT, "%02d", mins)
        val secsStr = String.format(Locale.ROOT, "%02d", secs)
        resources.getString(R.string.base_time_hms, sgnStr, hrsStr, minsStr, secsStr)
    } else if(mins > 0L) {
        val minsStr = String.format(Locale.ROOT, "%d", mins)
        val secsStr = String.format(Locale.ROOT, "%02d", secs)
        resources.getString(R.string.base_time_ms, sgnStr, minsStr, secsStr)
    } else {
        val secsStr = String.format(Locale.ROOT, "%d", secs)
        val msStr   = String.format(Locale.ROOT, "%03d", timeLeft)
...
fun timeToAlmostFullString(resources: Resources, timeInMS: Int64): String {
    ...
    val msStr = String.format(Locale.ROOT, "%02d", timeLeft/10L)
    return if(hrs > 0) {
        val hrsStr = String.format(Locale.ROOT, "%d", hrs)
        val minsStr = String.format(Locale.ROOT, "%02d", mins)
        val secsStr = String.format(Locale.ROOT, "%02d", secs)
...
fun timeToFullString(resources: Resources, timeInMS: Int64): String {
    var timeLeft = abs(timeInMS)
    val sgnStr   = if(timeInMS < 0) "-" else ""

    val hrs = timeLeft / 3600000L
    val hrsStr = String.format(Locale.ROOT, "%02d", hrs)
    timeLeft -= hrs * 3600000L
